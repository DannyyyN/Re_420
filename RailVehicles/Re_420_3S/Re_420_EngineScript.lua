...
--         Basiert auf NPZ Domino Architektur (B42Rail)     --
--------------------------------------------------------------
...
function SmoothMove(dt, cur, tgt, mul)
    if cur == tgt then return cur end
    if tgt > cur then
        cur = cur + dt * mul
        if cur > tgt then cur = tgt end
    else
        cur = cur - dt * mul
        if cur < tgt then cur = tgt end
    end
...
function SmoothMoveEaseOut(dt, cur, tgt, smoothness)
    if math.abs(cur - tgt) < 0.001 then return tgt end
    local diff = tgt - cur
    cur = cur + diff * smoothness * dt
    if (diff > 0 and cur > tgt) or (diff < 0 and cur < tgt) then cur = tgt end
    return cur
end
...
local function GetFSPos(tv)
    if     tv >  4.9 then return "++"
    elseif tv >  3.9 then return "+"
    elseif tv >  2.5 then return "M"
    elseif tv >  1.9 then return "dot"      -- Fahren ●
    elseif tv >  0.9 then return "minus"    -- Fahren –
    elseif tv > -0.1 then return "0"        -- Neutral 0
    elseif tv > -1.1 then return "b_minus"  -- Bremsen –
    elseif tv > -2.1 then return "b_dot"    -- Bremsen ●
    else                   return "b_plus"  -- Bremsen +
    end
end
        ...
        if G_BrakePressureGoal < -0.5 then
            ...
            if G_Fuellstellung_Active then
                ...
                elseif G_Fuellstellung_Dwell_Timer >= 2.0 then
                    if hauptluftdruck < 5.5 then hauptluftdruck = 5.5 end
                    G_PostFuellstellung_Descent = false
                else
                    G_PostFuellstellung_Descent = false
                end
...
function OnControlValueChange(name, value)
    local tv       = GetControlValue("VirtualThrottle") or 0
    local reverser = GetControlValue("Reverser") or 0
    local speedKPH = math.abs(Call("GetSpeed") or 0) * 3.6
        ...
        if reverserAtNeutral then
            ...
            if not G_Msg_ThrottleNoReverser and G_StartupComplete then
                SysCall("ScenarioManager:ShowAlertMessageExt",
                    "Wendeschalter / Directional switch",
